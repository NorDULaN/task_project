...
def get_cart_data(cart):
    ...
    if cart:
        cart_total = cart.get_base_total()
        cart.total = cart_total
        cart.save()

    for sale in cart.get_active_sales_v2():
        if not sale.name_display:
            if cart.promocode and cart.promocode.from_base_price:
                data['sales']['none'] = 0
            else:
                if 'none' in data['sales']:
                    data['sales']['none'] += sale.product.price_override - sale.get_price()
                else:
                    data['sales']['none'] = sale.product.price_override - sale.get_price()
        elif sale.name_display.value in data['sales']:
            if cart.promocode and cart.promocode.from_base_price:
                data['sales'][sale.name_display.value] = 0
            else:
                data['sales'][sale.name_display.value] += sale.product.price_override - sale.get_price()
        else:
            if cart.promocode and cart.promocode.from_base_price:
                data['sales'][sale.name_display.value] = 0
            else:
...
def assign_anonymous_cart(request, user, queryset=Cart.objects.all(), *args, **kwargs):
    token = request.get_signed_cookie(COOKIE_NAME, default=None)
    if not token_is_valid(token):
        return request
    cart = get_anonymous_cart_from_token(
